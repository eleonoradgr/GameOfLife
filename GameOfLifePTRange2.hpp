...
public:
    explicit GameOfLifePTRange2(uint32_t n = 30, uint32_t m = 30, uint32_t seed = 1234, float den = 0.3): GameOfLife{n, m, seed, den}{};
...
void GameOfLifePTRange2::play(uint16_t numIter, bool printGrid, uint8_t numw) {
    ...
    if (delta >= 1){
        ...
        auto compute_rows = [&](RANGE2 range) {
